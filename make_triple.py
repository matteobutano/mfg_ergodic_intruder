...
m_small = mfg.mfg('bottom_right_small')
m_intermediate = mfg.mfg('bottom_right_intermediate')
m_large = mfg.mfg('bottom_right_large')

print(m_small.gamma_1, m_small.gamma_2)
print(m_intermediate.gamma_1, m_intermediate.gamma_2)
...
m_intermediate.draw_density(clim = clim,save = True, axis = False, title = False, colorbar = False, scale = False)
...
m_small_im = plt.imread('gfx/bottom_right_small.png')
m_intermediate_im = plt.imread('gfx/bottom_right_intermediate.png')
m_large_im = plt.imread('gfx/bottom_right_large.png')


axs[0].imshow(m_small_im)
axs[0].axis('off')
axs[0].set_title(r' $(\tilde \gamma^{(1)} << 1,\;\tilde \gamma^{(2)} << 1)$',fontsize = fontsize)
axs[1].imshow(m_intermediate_im)
axs[1].set_title(r' $(\tilde \gamma^{(1)} << 1,\;\tilde \gamma^{(2)} >> 1)$',fontsize = fontsize)
axs[1].axis('off')
axs[2].imshow(m_large_im)
axs[2].set_title(r' $(\tilde \gamma^{(1)} >> 1,\;\tilde \gamma^{(2)} >> 1)$',fontsize = fontsize)
axs[2].axis('off')
